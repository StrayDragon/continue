...
exports.sharedConfigSchema = zod_1.default
    ...
    autoAcceptEditToolDiffs: zod_1.default.boolean(),
...
function salvageSharedConfig(sharedConfig) {
    ...
    if ("allowAnonymousTelemetry" in sharedConfig) {
        var val = zod_1.default.boolean().safeParse(sharedConfig.allowAnonymousTelemetry);
        if (val.success) {
            salvagedConfig.allowAnonymousTelemetry = val.data;
        }
    }
    if ("disableIndexing" in sharedConfig) {
        var val = zod_1.default.boolean().safeParse(sharedConfig.disableIndexing);
        if (val.success) {
            salvagedConfig.disableIndexing = val.data;
        }
    }
    if ("disableSessionTitles" in sharedConfig) {
        var val = zod_1.default.boolean().safeParse(sharedConfig.disableSessionTitles);
        if (val.success) {
            salvagedConfig.disableSessionTitles = val.data;
        }
    }
    if ("disableAutocompleteInFiles" in sharedConfig) {
        var val = exports.sharedConfigSchema.shape.disableAutocompleteInFiles.safeParse(sharedConfig.disableAutocompleteInFiles);
        if (val.success) {
...
function modifyAnyConfigWithSharedConfig(continueConfig, sharedConfig) {
    ...
    configCopy.tabAutocompleteOptions = __assign({}, configCopy.tabAutocompleteOptions);
    ...
    configCopy.ui = __assign({}, configCopy.ui);
    ...
    configCopy.experimental = __assign({}, configCopy.experimental);
