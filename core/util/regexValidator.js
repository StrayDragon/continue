...
function validateAndSanitizeRegex(query) {
    ...
    for (var _i = 0, problematicPatterns_1 = problematicPatterns; _i < problematicPatterns_1.length; _i++) {
        var check = problematicPatterns_1[_i];
        if (check.pattern.test(query)) {
            if (check.fix) {
                sanitizedQuery = check.fix(sanitizedQuery);
                warnings.push("Fixed: ".concat(check.issue));
            }
            else {
                warnings.push("Warning: ".concat(check.issue));
            }
        }
    }
    ...
    for (var _a = 0, ripgrepSpecificIssues_1 = ripgrepSpecificIssues; _a < ripgrepSpecificIssues_1.length; _a++) {
        var issue = ripgrepSpecificIssues_1[_a];
...
function looksLikeLiteralSearch(query) {
    ...
    var hasUnescapedMetachars = /[.*+?^${}()|[\]]/g.test(query);
    var hasEscapedMetachars = /\\[.*+?^${}()|[\]\\]/g.test(query);
    // Check for regex constructs, but be more specific about what we consider regex
    var hasEscapeSequences = /\\[dws]/g.test(query); // \d, \w, \s
    var hasQuantifiers = /\{[0-9,]+\}/g.test(query); // {2,4}, {3}
    // Check for bracket patterns and distinguish between character classes and array access
    var bracketPattern = /\[[^\]]*\]/g;
    var brackets = query.match(bracketPattern);
    var hasCharacterClasses = false;
    if (brackets) {
        hasCharacterClasses = brackets.some(function (bracket) {
            var inside = bracket.slice(1, -1); // Remove [ ]
            // Character class indicators: ranges (a-z), multiple chars, negation (^)
            return /-/.test(inside) || inside.length > 1 || inside.startsWith("^");
        });
    }
    var hasRegexConstructs = hasEscapeSequences || hasQuantifiers || hasCharacterClasses;
...
function prepareQueryForRipgrep(query) {
    ...
    if (looksLikeLiteralSearch(query)) {
        return {
            ...
            warning: "Query contained special regex characters and was escaped for literal text search",
        };
    }
    // Otherwise validate and sanitize as regex
    var validation = validateAndSanitizeRegex(query);
    return {
