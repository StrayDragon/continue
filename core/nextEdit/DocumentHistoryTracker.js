 ...
 */
var DocumentHistoryTracker = /** @class */ (function () {
    function DocumentHistoryTracker() {
     ...
     */
    DocumentHistoryTracker.getInstance = function () {
     ...
     */
    DocumentHistoryTracker.prototype.addDocument = function (documentPath, documentContent, ast) {
     ...
     */
    DocumentHistoryTracker.prototype.push = function (documentPath, documentContent, ast) {
        var astHistory = this.documentAstMap.get(documentPath);
        var documentHistory = this.documentContentHistoryMap.get(documentPath);
        if (!astHistory || !documentHistory) {
            console.error("Document ".concat(documentPath, " not found in AST tracker"));
     ...
     * @throws Error if the document doesn't exist in the tracker.
     */
    DocumentHistoryTracker.prototype.getMostRecentAst = function (documentPath) {
        var astHistory = this.documentAstMap.get(documentPath);
        if (!astHistory) {
            console.error("Document ".concat(documentPath, " not found in AST tracker"));
            return null;
        ...
        if (astHistory.length === 0) {
            console.error("Document ".concat(documentPath, " has no ASTs"));
            return null;
        }
     ...
     * @throws Error if the document doesn't exist in the tracker.
     */
    DocumentHistoryTracker.prototype.getMostRecentDocumentHistory = function (documentPath) {
        var documentHistory = this.documentContentHistoryMap.get(documentPath);
        if (!documentHistory) {
            console.error("Document ".concat(documentPath, " not found in AST tracker"));
            return null;
        ...
        if (documentHistory.length === 0) {
            console.error("Document ".concat(documentPath, " has no history"));
            return null;
        }
     ...
     */
    DocumentHistoryTracker.prototype.deleteDocument = function (documentPath) {
