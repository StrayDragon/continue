...
var lcs_js_1 = require("../../util/lcs.js");
var lineStream_js_1 = require("../filtering/streamTransforms/lineStream.js");
function rewritesLineAbove(completion, prefix) {
    var lineAbove = prefix
        .split("\n")
    ...
    var firstLineOfCompletion = completion
        .split("\n")
...
var MAX_REPETITION_FREQ_TO_CHECK = 3;
function isExtremeRepetition(completion) {
    var lines = completion.split("\n");
    if (lines.length < 6) {
        return false;
    }
    for (var freq = 1; freq < MAX_REPETITION_FREQ_TO_CHECK; freq++) {
        var lcs = (0, lcs_js_1.longestCommonSubsequence)(lines[0], lines[freq]);
        if (lcs.length > 5 || lcs.length > lines[0].length * 0.5) {
            var matchCount = 0;
            for (var i = 0; i < lines.length; i += freq) {
...
function postprocessCompletion(_a) {
    var completion = _a.completion, llm = _a.llm, prefix = _a.prefix, suffix = _a.suffix;
    // Don't return empty
    ...
    if (llm.model.includes("mercury") || llm.model.includes("granite")) {
        ...
        var prefixEnd = prefix.split("\n").pop();
        if (prefixEnd) {
            if (completion.startsWith(prefixEnd)) {
                completion = completion.slice(prefixEnd.length);
            }
            else {
                var trimmedPrefix = prefixEnd.trim();
                var lastWord = trimmedPrefix.split(/\s+/).pop();
